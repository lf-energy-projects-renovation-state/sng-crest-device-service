...
class FirmwareMapperTest {
    ...
    private val previousVersion = "01.10"
    private val version = "01.20"
    private val name = "RTU#DELTA#FROM#$previousVersion#TO#$version.txt"
    private val wrongName = "FOTA.txt"
    private val line0 =
    ...
    private val previousUUID = UUID.randomUUID()

    @Test
    fun mapFirmwareDTOToEntity() {
        val firmwareDTO = testFirmwareDTO()
        val previousFirmware = testPreviousFirmwareEntity()
        ...
        assertThat(result.previousFirmwareId).isEqualTo(previousUUID)
        ...
        assertThat(result.packets[0].packet).isEqualTo(line0)
        assertThat(result.packets[1].packet).isEqualTo(line1)
    }
    ...
    fun mapEntitiesToFirmwares() {
        val firmwareEntities = listOf(testFirmwareEntity(name))
        val previousFirmware = testPreviousFirmwareEntity()
        whenever(firmwareRepository.findById(previousUUID)).thenReturn(Optional.of(previousFirmware))

        val result = firmwareMapper.mapEntitiesToFirmwares(firmwareEntities)

        val firmware = result.firmwares.first()
        assertThat(firmware.name).isEqualTo(name)
        assertThat(firmware.version).isEqualTo(version)
        assertThat(firmware.type).isEqualTo(FirmwareType.device)
        assertThat(firmware.fromVersion).isEqualTo(previousVersion)
        assertThat(firmware.numberOfPackages).isEqualTo(1)
    }

    @Test
    fun mapEntitiesToFirmwaresThrowsException() {
        val firmwareEntities = listOf(testFirmwareEntity(wrongName))

        assertThrows<FirmwareException> { firmwareMapper.mapEntitiesToFirmwares(firmwareEntities) }
    }

    private fun testFirmwareDTO() = FirmwareDTO(name, listOf(line0, line1))

    private fun testFirmwareEntity(name: String): Firmware {
        val firmware = Firmware(uuid, name, version, previousUUID, mutableListOf())
        val packet = testFirmwarePacket(firmware)
        firmware.packets.add(packet)
        return firmware
    }

    private fun testPreviousFirmwareEntity(): Firmware =
        Firmware(previousUUID, name, previousVersion, UUID.randomUUID(), mutableListOf())

    private fun testFirmwarePacket(firmware: Firmware) = FirmwarePacket(firmware, 0, line0)
}
