...
class AnalogAlarmThresholdPort4ResultHandlerTest {
    ...
    @InjectMockKs private lateinit var resultHandler: AnalogAlarmThresholdPort4ResultHandler

    @Test
    fun handleSuccess() {
        val command = CommandFactory.analogAlarmThresholdsPort4InProgress()
    ...
    fun handleFailure() {
        val command = CommandFactory.analogAlarmThresholdsPort4InProgress()
        val message = MessageFactory.messageWithUrc(listOf("AL7:DLER"), "")
    ...
    companion object {
        ...
        fun hasSucceededTestSource(): Stream<Arguments> =
            Stream.of(
                Arguments.of(listOf("AL7:SET"), "0", true),
                Arguments.of(listOf("INIT", "WDR"), "0", false),
                Arguments.of(listOf("INIT", "WDR", "AL7:SET"), "0", true),
            )

        @JvmStatic
        fun hasFailedTestSource(): Stream<Arguments> =
            Stream.of(
                Arguments.of(listOf("AL7:SET"), "0", false),
                Arguments.of(listOf("INIT", "WDR"), "0", false),
                Arguments.of(listOf("AL7:DLER"), "0", true),
                Arguments.of(listOf("AL7:ERR"), "0", true),
            )
    }
}
